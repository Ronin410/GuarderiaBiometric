...
3. Variables opcionales:
   ...
   - `ANTHROPIC_API_KEY`, `VOYAGE_API_KEY` — chat de soporte con IA (RAG): antes de avisarle al dueño de la plataforma, el asistente intenta contestar solo usando la documentación de los manuales de usuario como contexto (ver `internal/server/ia_soporte.go`). Sin las dos, el chat de soporte se comporta exactamente igual que sin esta función (siempre avisa de inmediato). Requiere además:
     - Que la instancia de Postgres de `DATABASE_URL` tenga disponible la extensión `pgvector` (en Render hay que habilitarla desde el dashboard de la base de datos antes del primer deploy con la migración `000043_conocimiento_ia`).
     - Haber corrido `go run ./cmd/ingest-conocimiento -db "$DATABASE_URL" -voyage-key "$VOYAGE_API_KEY" ../GuarderiaBiometricFront/public/manual.html ../GuarderiaBiometricFront/public/manual-papa.html` al menos una vez (y de nuevo cada vez que cambie el contenido de esos manuales) para llenar la base de conocimiento.
