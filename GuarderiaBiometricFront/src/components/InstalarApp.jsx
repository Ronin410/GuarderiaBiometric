...
// InstalarApp se monta en DashboardPadre (el portal del papá) -- es la
...
const InstalarApp = () => {
  ...
  if (!plataforma.esNavegadorEnApp && !promptDiferido && !plataforma.esIOS) return null;
  ...
  return (
    <>
        ...
        <div className="flex-1 pr-4">
          {plataforma.esNavegadorEnApp ? (
            ...
            <>
              ...
              <p className="text-white/60 text-xs mt-1 mb-3">Entra más rápido, sin buscar el link cada vez.</p>
              <button onClick={() => setMostrarPasosIOS(true)} className="bg-brand-600 hover:bg-brand-700 text-white text-xs font-black uppercase px-4 py-2.5 rounded-xl transition-all active:scale-95">
                Ver cómo (2 pasos)
              </button>
            </>
          )}
      ...
      {mostrarPasosIOS && (
        <div className="fixed inset-0 z-[300] flex items-end sm:items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm" onClick={() => setMostrarPasosIOS(false)}>
          <div className="bg-white rounded-[2rem] w-full max-w-sm p-6 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-black text-slate-900">Instalar en tu iPhone</h3>
              <button onClick={() => setMostrarPasosIOS(false)} className="text-slate-400 hover:text-slate-600 p-1"><X size={22} /></button>
            </div>

            <div className="space-y-4">
              <div className="flex items-center gap-4 bg-slate-50 rounded-2xl p-4">
                <div className="bg-white text-brand-600 w-11 h-11 rounded-2xl flex items-center justify-center font-black text-lg shrink-0 shadow-sm">1</div>
                <IconoCompartirIOS size={30} className="text-blue-500 shrink-0" />
                <span className="text-sm font-bold text-slate-700 leading-snug">Toca el ícono de Compartir, abajo en la pantalla</span>
              </div>
              <div className="flex items-center gap-4 bg-slate-50 rounded-2xl p-4">
                <div className="bg-white text-brand-600 w-11 h-11 rounded-2xl flex items-center justify-center font-black text-lg shrink-0 shadow-sm">2</div>
                <IconoAgregarInicio size={30} className="text-slate-500 shrink-0" />
                <span className="text-sm font-bold text-slate-700 leading-snug">Busca y toca "Agregar a inicio"</span>
              </div>
            </div>

            <p className="text-center text-xs text-slate-400 font-bold mt-6">Listo -- aparecerá en tu pantalla de inicio como una app.</p>
          </div>
        </div>
      )}
    </>
