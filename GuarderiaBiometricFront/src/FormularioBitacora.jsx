    ...
    <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-100 overflow-hidden animate-in zoom-in duration-300 max-w-4xl w-full mx-auto">
      <div className="bg-brand-600 p-8 text-white flex justify-between items-center">
        <div>
          ...
          <p className="text-brand-200 text-xs font-bold uppercase tracking-widest mt-2">{nombreNiño}</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex items-center justify-between bg-slate-50 p-5 rounded-[2rem] border border-slate-100">
            <div className="flex items-center gap-4">
              <div className={`p-3 rounded-xl ${formData.durmio ? 'bg-brand-600 text-white shadow-lg shadow-brand-200' : 'bg-slate-200 text-slate-400'}`}>
                <Moon size={20} />
              </div>
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              ...
              <div className="w-12 h-7 bg-slate-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[4px] after:left-[4px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-brand-600"></div>
            </label>
          </div>
          ...
          <div className="flex flex-col gap-3 bg-slate-50 p-5 rounded-[2rem] border border-slate-100">
             <label className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400 tracking-widest">
               <CheckCircle2 size={14} className="text-brand-500" /> Esfínter
             </label>
             <div className="flex gap-2">
                {['PIPI', 'POPO'].map(tipo => (
                  <button
                    ...
                    className={`flex-1 py-2 rounded-xl text-[10px] font-black transition-all border ${formData.esfinter.includes(tipo) ? 'bg-brand-600 text-white border-brand-600 shadow-md' : 'bg-white text-slate-400 border-slate-200'}`}
                  >
                    {tipo}
                  </button>
                ))}
        ...
        <div className="flex flex-col gap-2">
          <label className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400 tracking-widest ml-2">
            <Info size={14} className="text-brand-500" /> Observaciones Generales
          </label>
          <textarea
            className="w-full bg-slate-50 border border-slate-100 rounded-[2.5rem] p-6 text-sm font-medium outline-none focus:ring-2 focus:ring-brand-500 transition-all min-h-[120px] resize-none shadow-inner"
        ...
        <div className="space-y-4">
          <label className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400 tracking-widest ml-2">
            <ImageIcon size={14} className="text-brand-500" /> Evidencia Fotográfica
          </label>
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            <label className="aspect-square bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2rem] flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-slate-100 hover:border-brand-300 transition-all group">
            ...
            {fotosExistentes.map((url, index) => (
              <div key={`exp-${index}`} className="aspect-square rounded-[2rem] overflow-hidden relative border-2 border-brand-100">
        ...
        <div className="flex gap-4 pt-4">
          <button type="submit" className="flex-1 bg-brand-600 hover:bg-brand-700 text-white font-black uppercase tracking-widest py-5 rounded-[2rem] shadow-xl shadow-brand-200 flex items-center justify-center gap-3 transition-all active:scale-95">
