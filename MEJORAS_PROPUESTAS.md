...
**Sección 1 (Seguridad)** — hecho: JWT secret por env var, log de contraseñas eliminado, `pin_admin` ya no se devuelve en `/login`, CORS restringido vía `ALLOWED_ORIGINS`, rate limiting en `/login`/`/identificar`/`/verificar-pin`, `RequireStaff()` bloqueando `/admin/ninos`/editar perfil/todo `/pagos*`/`/reportes/asistencia-resumen` para cuentas rol `papa`, `collection_id` inconsistente ya no se envía desde el frontend (el backend siempre lo recalcula), el enlace público de bitácora ahora se puede revocar bajo demanda (`POST /hijos/:id/regenerar-token`, botón "Regenerar enlace" en Familia — no expira por tiempo a propósito, es un link de uso diario, no de un solo uso), y la sesión avisa proactivamente antes de expirar en vez de solo fallar con 401. Pendiente (fuera de alcance por ahora, es un cambio de arquitectura de auth más grande): migrar el JWT de `localStorage` a cookies `httpOnly`.
